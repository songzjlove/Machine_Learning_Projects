...
# Compute the gist descriptor and normalize them.
...
# problem 3
## Note: Codes without parallelization are not desirable, because the computation complexity is O(N^2 * M), 
...
# Assign "evenly" heavy burdens for engines
reordered_li = [] 
down = N
up = 0
for i in range(Num):
...
# Another way to parallelize the computation by block. Map
@dview.remote(block = True)
def wc():
    import numpy as np
    #distance_map = {}
...
# Reduce the results into the Distance_Matrix
wcs = wc()
...
# Learned lesson:If you can handle the program in 5 nodes cluster, never use the bullshit 10 nodes one!!!
