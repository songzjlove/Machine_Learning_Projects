...
# This algorithm should be developed with looking up more specials cases and adjust the strateties insides. We believe the thoughts are right.
# 
#     
...
for i in range(start,m):
    ...
    checkpoint.store( project_name+"_wave3", obj = top_wave3)
    finished_top_ids += len(temp)
...
print len(new_wave1 + new_wave2) + len(new_wave0)+len(new_wave3)
...
end_of_processed = len(new_wave1 + new_wave2) + len(new_wave0)+len(new_wave3)
print end_of_processed
new_ranking = []
left_ranking = ranking[end_of_processed:]

new_ranking.extend(new_wave3)
...
RESULT = new_ranking
with open(project_name+'_ff.txt', 'w+') as f:
    for Re in RESULT:
        f.write(str(Re)+'\n')
f.close()
